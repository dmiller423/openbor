...
void ImGui_ImplSoft1_RenderDrawLists(ImDrawData* draw_data)
{
	VideoOut *videoOut = VideoOut::Get();

	ImGuiIO& io = ImGui::GetIO();
	uint32_t* pixels = (uint32_t*)io.UserData;

	paint_imgui(pixels, videoOut->Width(), videoOut->Height(), sw_options);

	memcpy((void*)videoOut->CurrentBuffer(), pixels, videoOut->BufferSize());
}


IMGUI_API bool SoftRend1::ImGui_Init(void *initData)
{
	ImGuiIO& io = ImGui::GetIO();
	io.UserData = (void*)rendBuffer;
	io.RenderDrawListsFn = ImGui_ImplSoft1_RenderDrawLists;
	...
	return true; //ImGui_ImplSoft2_Init(initData);
}

IMGUI_API void SoftRend1::ImGui_Shutdown()
{
	imgui_sw::unbind_imgui_painting();
}

IMGUI_API void SoftRend1::ImGui_NewFrame(void *frameData)
{
	ImGui::NewFrame();
}

#if 0
IMGUI_API bool SoftRend2::ImGui_ProcessEvent(void* eventData)
{
	return ImGui_ImplSoft2_ProcessEvent(eventData);
}
#endif


// Use if you want to reset your rendering device without losing ImGui state.
IMGUI_API bool SoftRend1::ImGui_CreateDeviceObjects()
{
	return true;
}


IMGUI_API void SoftRend1::ImGui_InvalidateDeviceObjects()
{
